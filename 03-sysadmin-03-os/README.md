...
3. Предположим, приложение пишет лог в текстовый файл. Этот файл оказался удален (deleted в lsof), однако возможности сигналом сказать приложению переоткрыть файлы или просто перезапустить приложение – нет. Так как приложение продолжает писать в удаленный файл, место на диске постепенно заканчивается. Основываясь на знаниях о перенаправлении потоков предложите способ обнуления открытого удаленного файла (чтобы освободить место на файловой системе).
   Ответ:  
 ```
 ps au | grep vim # нашли что pid 4141

ll /proc/4141/fd # нашли номер файлового дескриптора

/proc/4141/fd$ echo '' > /proc/4141/fd/5
 ```
...
3. Чем отличается последовательность команд через `;` и через `&&` в bash? Например:
    ...
    Ответ:
     ;  выполняются последовательно, независимо от результата работы предыдущей.

     && следующая команда выполнится только, если предшествующая завершится успешно.
...
Ответ:  Дополнительные буквы статуса - это приоритет, многопоточность
```
root@vagrant:/home/vagrant# ps a -o stat
+ ps a -o stat
STAT
Ss
S
