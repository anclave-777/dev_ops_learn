...
Ответ:
Нет не могут. Это 2 разных флага состояния TCP сессии
...
Ответ:
Потому что нет соединения с хостом. Поскольку речь идет о udp тут не может быть состояние TIME-WAIT.
